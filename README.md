...
- **Answer formats per round:** single column, two columns (song and artist), true/false, multiple choice (2-6 choices, with the real answer text printed next to each letter once you've added it), or a picture round with your own images in numbered frames.
...
| `options` | no, Multiple Choice only | That question's answer choices, comma-separated (e.g. `Lions, Tigers, Bears, Oh My`) — prints as `A Lions  B Tigers  C Bears  D Oh My` instead of bare lettered circles |
...
**No spreadsheet handy?** A Multiple Choice round also has its own compact box in section 3 — one line per question, its choices comma-separated — for typing options straight into the app. A file's `options` column always wins for a question it covers; the box only fills in whatever the file doesn't.
